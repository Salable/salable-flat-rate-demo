...
const Main = ({uuid}: {uuid: string}) => {
  ...
  const {data: invoices, mutate: mutateInvoices, isLoading: isLoadingInvoices} = useSWR<GetAllInvoicesResponse>(`/api/subscriptions/${uuid}/invoices`)
  ...
  useEffect(() => {
    if (isPolling) {
      ...
      if (isChangingSubscription) {
        const subscriptionPolling = setInterval(async () => {
          try {
            const res = await fetch(`/api/subscriptions?email=${session?.email}&planUuid=${changingPlanUuid}&status=active`)
            const data = await res.json() as GetAllLicensesResponse
            if (data?.data.length && data?.data[0].planUuid === changingPlanUuid) {
              clearInterval(subscriptionPolling)
              setIsPolling(false)
              await mutateSubscription()
  ...
  }, [pollSubscription, changingPlanUuid, isReactivatingSubscription]);
  ...
  return (
    <>
      <div className='max-w-[1000px] m-auto'>
        <div>
          {!isLoadingSubscription && !isLoadingInvoices ? (
            <div>
              <h1 className='text-3xl mb-6 flex items-center'>Subscription
                {!isValidatingSubscription ? (
                  ...
                  <div className="ml-2 h-[34px] w-[95px] bg-slate-300 rounded-md"></div>
                )}
              </h1>
                      ...
                      {!subscription?.cancelAtPeriodEnd ? (
                        <>
                          <button
                            ...
                            onClick={async () => {
                              await changeSubscription(subscription?.planUuid === salableProPlanUuid ? salableBasicPlanUuid : salableProPlanUuid)
                            }}
                          ...
                          >
                            ...
                            Change to {subscription?.planUuid === salableProPlanUuid ? "Basic" : "Pro"}
                          </button>
