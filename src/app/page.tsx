...
import {appBaseUrl, salableBasicPlanUuid, salableProPlanUuid, salableProductUuid} from "@/app/constants";
...
const Main = () => {
  ...
  const {data: licenseCheck, isLoading: isLoadingLicenseCheck, isValidating: isValidatingLicenseCheck, mutate: mutateLicenseCheck} = useSWR<LicenseCheckResponse>(`/api/licenses/check`)
  const {data: currentUsage, isLoading: isLoadingCurrentUsage, isValidating: isValidatingCurrentUsage, mutate: mutateCurrentUsage} = useSWR<{unitCount: number; updatedAt: string}>(`/api/usage/current?planUuid=${licenseCheck?.capabilitiesEndDates?.['128'] ? salableProPlanUuid : salableBasicPlanUuid}`)

  const upgradeToPro = async () => {
    try {
      setDisableButton(true)
      setIsChangingSubscription(true)
      const licensesRes = await fetch(`/api/licenses?planUuid=${salableBasicPlanUuid}&status=active&granteeId=${session?.uuid}`)
      ...
      const change = await fetch(`/api/subscriptions/${licenses.data[0].subscriptionUuid}/change`, {
        method: 'PUT',
        body: JSON.stringify({planUuid: salableProPlanUuid})
      })
      if (change.ok) {
        setIsPolling(true)
        setChangingPlanUuid(salableProPlanUuid)
      } else {
              ...
              <div className='grid grid-cols-3 gap-6'>
                ...
                <div className='p-6 rounded-lg bg-white shadow flex-col'>
                  ...
                  <div className='flex'>
                    {!isLoadingSession && !session?.uuid ? (
                      <Link
                        href={`/sign-up?planUuid=${salableBasicPlanUuid}&successUrl=${appBaseUrl}`}
                        className='block p-4 text-white rounded-md leading-none bg-blue-700 w-full text-center'
                      >
                        Sign up
                      </Link>
                    ) : (
                      <PlanButton uuid={salableBasicPlanUuid} successUrl={appBaseUrl} />
                    )}
                  </div>
                </div>
                ...
                <div className='p-6 rounded-lg bg-white shadow flex-col'>
                  ...
                  <div>
                    {!isLoadingSession && !session?.uuid ? (
                      <Link
                        href={`/sign-up?planUuid=${salableBasicPlanUuid}&successUrl=${appBaseUrl}/usage`}
                        className='block p-4 text-white rounded-md leading-none bg-blue-700 w-full text-center'
                      >
                        Sign up
                      </Link>
                    ) : (
                      <PlanButton uuid={salableBasicPlanUuid} successUrl={`${appBaseUrl}/usage`}/>
                    )}
                  </div>
                </div>
